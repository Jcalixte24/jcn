...
const Hub3DScene = () => {
  ...
  return (
    <div id="hub3d-container" className="relative w-full h-screen overflow-hidden bg-background touch-none">
      ...
      {/* Section Content Overlay — independent scroll */}
      <AnimatePresence>
        {activeSection && ActiveComponent && (
          ...
          >
            ...
            />
            <motion.div
              className="relative z-10 w-full max-w-4xl max-h-[88vh] flex flex-col rounded-2xl border overflow-hidden"
              style={{
                borderColor: activeSectionData ? `${activeSectionData.color.replace(")", " / 0.2)")}` : "hsl(var(--border) / 0.3)",
                background: "var(--gradient-card)",
                backdropFilter: "blur(24px)",
                boxShadow: activeSectionData
                  ? `0 0 60px ${activeSectionData.color.replace(")", " / 0.1)")}, var(--shadow-xl), inset 0 1px 0 hsl(0 0% 100% / 0.05)`
                  : "var(--shadow-xl), inset 0 1px 0 hsl(0 0% 100% / 0.05)",
              }}
            ...
            >
              {/* Sticky header */}
              <div
                className="sticky top-0 z-20 flex items-center justify-between px-4 py-2.5 border-b border-border/20 rounded-t-2xl flex-shrink-0"
                style={{ background: "hsl(var(--card) / 0.9)", backdropFilter: "blur(16px)" }}
              >
                <div className="flex items-center gap-2">
                  <span className="text-sm">{activeSectionData?.icon}</span>
                  <span
                    className="text-[9px] font-exo tracking-widest uppercase font-semibold"
              ...
              {/* Scrollable content — independent from 3D scroll */}
              <div className="flex-1 overflow-y-auto cosmic-scrollbar overscroll-contain">
                <div className="pb-8">
                  <ActiveComponent />
                </div>
              </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 h-8 pointer-events-none rounded-b-2xl" style={{
