...
const SectionWaypoint = ({
  ...
  // On mobile, center waypoints horizontally for better visibility
  const adjustedPosition: [number, number, number] = mobile
    ? [position[0] * 0.3, position[1] * 0.5, position[2]]
    : position;
  ...
  useFrame((state) => {
    ...
    // Gentle float
    groupRef.current.position.y = adjustedPosition[1] + Math.sin(t * 0.8 + adjustedPosition[2] * 0.1) * 0.15;
    groupRef.current.position.x = adjustedPosition[0] + Math.sin(t * 0.3 + adjustedPosition[2] * 0.05) * 0.08;

    // Make the group face the camera (billboard effect for visibility)
  ...
  return (
    <group ref={groupRef} position={adjustedPosition}>
      <Html
        transform
        distanceFactor={mobile ? 6 : 6.5}
        style={{ pointerEvents: isNear ? "auto" : "none" }}
        occlude={false}
      >
        <div
          className={`
            ${mobile ? "w-[140px] p-3" : "w-[150px] p-4"} rounded-xl cursor-pointer select-none
